...
class SimpleNet(torch.nn.Module):
    ...
    def load(
        ...
        **kwargs,
        ...
        if self.pre_proj > 0:
            ...
            self.projection_extra = Projection(self.target_embed_dimension, self.target_embed_dimension, 1, 0)
            self.projection_extra.to(self.device)
            self.proj_opt_extra = torch.optim.AdamW(self.projection_extra.parameters(),lr*.1)
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False, evaluation=False):
        ...
        # print("CNN+ViT feature shape:", features[0].shape)
        # for i, f in enumerate(features):
            # print(f"[Layer {i}] feature shape: {f.shape}")
        ...
        if hasattr(self, "projection_extra"):
            features = self.projection_extra(features)
